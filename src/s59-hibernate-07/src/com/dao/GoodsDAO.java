...
public class GoodsDAO extends BaseHibernateDAO {
	...
	public Goods findById(java.lang.Integer id) {
		...
		try {
			Goods instance = (Goods) getSession().get("com.pojo.Goods", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(Goods instance) {
		...
		try {
			List results = getSession().createCriteria("com.pojo.Goods")
	...
	public List<Goods> fenye(int page, int size){
		...
		query.setFirstResult((page-1)*size);//从第几条开始查
		query.setMaxResults(size);//查几条
		List<Goods> list = query.list();//查找出来的结果就是list
		return list ;
	}
	...
	public int getCount(){
		...
		int count = (int)(long)(Long) query.uniqueResult();// 如果是聚合查询
		return count ;
	}
}